...
import '../data/model/response/statecity.dart';
...
class LocationProvider with ChangeNotifier {
  ...
  StateCityData? _stateCityData;
  RegisterZipModel? _zipSelected;
  List<RegisterZipModel> _registerZipList = [];
  List<RestrictedZipModel> get restrictedZipList => _restrictedZipList;
  List<RegisterZipModel> get registerZipList => _registerZipList;
  List<RegisterZipModel> filteredZipList = [];
  RegisterZipModel? selectedZip;

  StateCityData? get stateCityData => _stateCityData;
  RegisterZipModel? get zipSelected => _zipSelected;
  List<RegisterZipModel> get registerCityList => registerCityList;
  ...
  void filterVendors(String value) {
    print('fbfdgfdgfdg${value}');
    filteredZipList = _registerZipList!
        .where((vendor) =>
            vendor.zipcode!.toLowerCase().contains(value.toLowerCase()))
        .toList();
  }
  ...
  Future<ResponseModel?> getRestrictedZipCode(BuildContext context) async {
    ResponseModel? responseModel;
    ApiResponse apiResponse = await locationRepo!.getZipcode();
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      _registerZipList = [];
      // Map mydata = jsonDecode(apiResponse.response!.data);
      List myzipList = apiResponse.response!.data['data'];
      print("objectmyzipList ${myzipList.length}");
      myzipList.forEach((address) =>
          _registerZipList.add(RegisterZipModel.fromJson(address)));
      filterVendors('');

      // _zipSelected = _registerZipList[0];
      responseModel = ResponseModel('successful', true);
      notifyListeners();
    } else {
      if (context.mounted) {}
  ...
  setZipCodeSelected(RegisterZipModel registerZipModel) {
    _zipSelected = registerZipModel;
    notifyListeners();
  }
  ...
  // setZipCodeSelected2(String zipcode){
  //
  //   __registerZipList.firstWhere()
  //   _zipSelected = registerZipModel;
  //   notifyListeners();
  // }
  ...
  Future<void> getDeliveryRestrictedCityByZip(
      BuildContext context, String zipcode) async {
    ApiResponse response =
        await locationRepo!.getDeliveryRestrictedCityByZip(zipcode);
    if (response.response!.statusCode == 200) {
      _stateCityData = StateCityData.fromJson(response.response!.data);
    } else {
      if (context.mounted) {}
      ApiChecker.checkApi(response);
    }
