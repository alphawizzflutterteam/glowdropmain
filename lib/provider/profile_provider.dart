...
class ProfileProvider extends ChangeNotifier {
  ...
  double? _reBalance;
  double? get reBalance => _reBalance;
  String? get selectedCity => _selectedCity;
  List<String> get addressTypeList => _addressTypeList;
  String? get addressType => _addressType;
  UserInfoModel? get userInfoModel => _userInfoModel;
  KYCModel? get kycInfoModel => _kycInfoModel;
  ...
  void setSelectCity(String city) {
    _selectedCity = city;
    notifyListeners();
  }
  ...
  Future<void> initAddressList() async {
  ...
  void initKYC(int? id, int index, BuildContext context) async {
    _isLoading = true;
    notifyListeners();
    ApiResponse apiResponse = await profileRepo!.removeAddressByID(id);
    if (apiResponse.response != null &&
  ...
  Future<String> getUserInfo(BuildContext context) async {
    ...
    if (apiResponse.response != null &&
      ...
      _reBalance = _userInfoModel!.repurchaseWallet;
    } else {
      ApiChecker.checkApi(apiResponse);
    }
    notifyListeners();
  ...
  Future<KYCModel?> getUserKYC(BuildContext context, String userId) async {
    String userID = '-1';
    print('_kycInfoModel ${_kycInfoModel}');
    ApiResponse apiResponse = await profileRepo!.getKYC(userId);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      _kycInfoModel = KYCModel.fromJson(apiResponse.response!.data["data"]);
      print('_kycInfoModel ${_kycInfoModel}');
      print('_kycInfoModel ${_kycInfoModel!.status}');
      userID = _kycInfoModel!.user_id.toString();
      // _balance = _userInfoModel!.walletBalance;
    } else {
      ApiChecker.checkApi(apiResponse);
    }
  ...
  Future<void> initAddressTypeList(BuildContext context) async {
  ...
  Future<ResponseModel> updateKYC(KYCModel updateUserModel, File? pan,
    ...
    http.StreamedResponse response = await profileRepo!
        .updateKYCR(updateUserModel, pan, addharf, addharb, passBook, token);
    _isLoading = false;
    if (response.statusCode == 200) {
      Map map = jsonDecode(await response.stream.bytesToString());
      print("object RESPOMSE $map");
      bool? message = map["status"];
      // _userInfoModel = updateUserModel;
      responseModel = ResponseModel(map.toString(), true);
      if (kDebugMode) {
        print(message);
      }
    } else {
