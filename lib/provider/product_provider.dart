...
class ProductProvider extends ChangeNotifier {
  ...
  Future<void> getLatestProductList(int offset, {bool reload = false}) async {
    ...
    if (!_offsetList.contains(offset)) {
      ...
      ApiResponse apiResponse = await productRepo!.getLatestProductList(
          Get.context!, offset.toString(), productType, title);
  ...
  Future<int?> getLatestOffset(BuildContext context) async {
    ApiResponse apiResponse = await productRepo!
        .getLatestProductList(context, '1', productType, title);
  ...
  Future<ApiResponse> initSellerProductList(
    ...
    ApiResponse apiResponse = await productRepo!
        .getSellerProductList(sellerId, offset.toString(), search);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      if (reload) {
        _sellerAllProductList = [];
  ...
  void initBrandOrCategoryProductList(
    ...
    _hasData = true;
    ApiResponse apiResponse = await productRepo!
        .getBrandOrCategoryProductList(isBrand, id, _maxPrice, _minPrice);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      print('object');
      ...
      if (_filterIndex == 1) {
        _brandOrCategoryProductList.sort(
            (a, b) => a.name!.toLowerCase().compareTo(b.name!.toLowerCase()));
      } else if (_filterIndex == 2) {
        _brandOrCategoryProductList.sort(
            (a, b) => a.name!.toLowerCase().compareTo(b.name!.toLowerCase()));
        Iterable iterable = _brandOrCategoryProductList.reversed;
        _brandOrCategoryProductList = iterable.toList() as List<Product>;
  ...
  bool? _hasServiceData;

  List<ServiceData> get serviceList => _serviceList;
  bool? get hasServiceData => _hasServiceData;

  void initServiceList(bool isBrand, String id, BuildContext context,
      {String? text}) async {
    print('fsjfasfjsad');
    _serviceList.clear();
    ...
    ApiResponse apiResponse =
        await productRepo!.getServiceList(isBrand, id, text ?? "");
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
      print('safasfafaffa${apiResponse.response!.data}');
      var data = apiResponse.response!.data;
      _serviceList =
          (data['data'] as List).map((e) => ServiceData.fromJson(e)).toList();
      _hasServiceData = false;
      // apiResponse.response!.data.forEach((product) =>
      //     _serviceList.add(ServiceData.fromJson(product)));
      // _hasData = _serviceList.length > 1;
      // List<ServiceData> products = [];
      // products.addAll(_serviceList);
      // _serviceList.clear();
      // _serviceList.addAll(products.reversed);
    } else {
      _hasServiceData = false;
      ApiChecker.checkApi(apiResponse);
    }
