...
class PriceConverter {
  ...
  static String convertPrice1(BuildContext context, double? price, {double? discount, String? discountType}) {
    if (discount != null && discountType != null) {
      if (discountType == 'amount' || discountType == 'flat') {
        price = price! - discount;
      } else if (discountType == 'percent' || discountType == 'percentage') {
    ...
    price = singleCurrency
        ? price
        : price! *
        Provider.of<SplashProvider>(context, listen: false).myCurrency!.exchangeRate! *
        (1 / Provider.of<SplashProvider>(context, listen: false).usdCurrency!.exchangeRate!);

    // Return only the numeric value without formatting
    return price!.toStringAsFixed(0); // No decimals, just the integer part
  }
