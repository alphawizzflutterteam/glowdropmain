...
import '../model/body/rgistermodel.dart';
...
class AuthRepo {
  ...
  // Future<ApiResponse> submitUserDetails(UserDetailsModel userDetails) async {
  //   try {
  //     Response response = await dioClient!.post(
  //       AppConstants.userDetailsUri,
  ...
  Future<ApiResponse> login(LoginModel loginBody) async {
    print('parameter------${loginBody.toJson()}');
    print("Response ${AppConstants.loginUri}");
    try {
      Response response = await dioClient!.post(
  ...
  Future<ApiResponse> loginWithOtp(LoginWithOtpModel loginBody) async {
    print('parameter------${loginBody.toJson()}');
    print("Response ${AppConstants.loginUri}");
    try {
      Response response = await dioClient!.post(
        "https://glow-drop.developmentalphawizz.com/api/v1/auth/verify-phone",
        data: loginBody.toJson(),
      );
  ...
  // for  user token
  Future<void> saveIndex(String token) async {
    try {
      await sharedPreferences!.setString(AppConstants.AddessIndex, token);
    } catch (e) {
      rethrow;
    }
  }
  ...
  String getSaveIndex() {
    return sharedPreferences!.getString(AppConstants.AddessIndex) ?? "";
  }
